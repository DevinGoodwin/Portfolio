...
public class Sudoku {    
    ...
    private boolean isValidNum(int n,int row,int col){
        if(subgridHasVal[row/3][col/3][n]||rowHasVal[row][n]||colHasVal[col][n]){
            return false;
        }
        return true;
    }
           ...
           for(int col = 0; col < n; col++){
               if(this.grid[row][col]==0){
                   for(int number = 1; number <= n;number++){
                       if(isValidNum(number, row, col)){
                           placeVal(number, row, col);
                           if(solveRB(n)){
                            
                               return true;
                           }else{
                            removeVal(number, row, col);
                           }
                       }
                   }
                return false;
    ...
    public boolean solve() { 
        boolean foundSol = this.solveRB(9);
        return foundSol;
    }
